...
def report_markdown(report):
    """Serialise the inference report into a readable Markdown case summary (for export)."""
    cdr, ipdr = report.get("cdr", {}), report.get("ipdr", {})
    L = ["# ARGUS — Case Analysis Report", "",
         f"- CDR phone subjects: **{cdr.get('subjects', 0)}**",
         f"- IPDR sessions: **{ipdr.get('sessions', 0)}**"]
    ...
    def leaderboard(title, rows):
        ...
        if not rows:
            L.append("_none_")
        for r in rows[:15]:
            L.append(f"- **{r['subject']}** — {r['score']}/100 ({r['band']})")
    ...
    ents = cdr.get("entities", [])
    if ents:
        L += ["", "## Likely multi-SIM identities"]
        L += [f"- {', '.join(e['numbers'])} ({e['confidence']} confidence)" for e in ents[:15]]
    L.append("")
    L.append("_Generated by ARGUS. Each item is an investigative lead to verify, not proof._")
    return "\n".join(L)
