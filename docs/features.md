...
### Spatiotemporal Inference Engine (`inference_service.py`, `geo.py`)
- **Description:** Builds a **per-subject, movement-annotated unified timeline** — each subject's calls, SMS and data sessions merged chronologically and keyed by msisdn (fixing the older timeline's mix of phone-number and IP identity spaces) — then derives investigative inferences. **(A) Movement:** distance/Δt/implied-speed between consecutive located events, with **travel-mode banding** (stationary → walking → road/highway → rail/expressway → air → impossible), **impossible-travel** flags (>900 km/h over ≥5 km), and **home/work anchors** (night-hours vs weekday-daytime dominant towers). **(B) Co-presence:** two subjects at the same tower (or within 1.5 km) inside a 20-min window, aggregated per pair — **convoy** when repeated across ≥2 days, and **"met but never called" hidden links** when co-located pairs never appear in the call graph. **(C) Behavioral:** per-subject activity **bursts** (daily z-score), **odd-hours** share, **periodic-contact** cadence (low coefficient-of-variation of inter-call gaps), and **going-dark** (adoption of VPN/Tor tunnel traffic — detected by category *or* tunnel/proxy destination port, so a VPN run on a cloud/VPS IP is still caught; plain cloud use is not treated as going dark). **(E) Possible VPN/proxy use:** a scored heuristic combining explicit tunnel ports (WireGuard/OpenVPN/IPsec/L2TP/PPTP), Tor/proxy ports, and **traffic concentration** — a VPN/proxy funnels most bytes through a single cloud/VPS endpoint, unlike normal browsing that fans out across many CDNs, so a dominant single-host share flags even stealth proxies on 443 (gated by a minimum session count so a lone session can't trivially flag). **(D) Identity/device:** **SIM swap/clone** (one number on multiple IMEIs), **burner** (one IMEI cycling numbers), and **clone corroboration** (impossible travel + multi-handset). Device anomalies key on msisdn, not IMSI, because IMSI is intentionally reused across the dummy CDR/IPDR files — keying on it would report a data artifact rather than a real anomaly.
- **Honest limits (documented in code):** tower coordinates approximate handset location (cell radius up to several km) and start times are minute-resolution, so speeds are area-to-area estimates; every inference is a graded signal, not proof.
- **Endpoints:** `GET /inference/report` (full report across all groups) and `GET /inference/subject/{subject}` (one subject's movement-annotated timeline + anchors + impossible legs). `build_unified_timeline` also now attaches a per-call `move` block.
- **Tech:** `build_subject_streams()` / `annotate_movement()` foundation; `subject_movement()`, `infer_anchors()`, `impossible_travel()`, `co_presence()`, `activity_bursts()`, `odd_hours_profile()`, `periodic_contacts()`, `going_dark()`, `vpn_proxy_use()`, `device_anomalies()`, `clone_corroboration()`, orchestrated by `run_all()`; `geo.haversine_km()` / `classify_speed()`; pure functions covered by `tests/test_inference.py`. Endpoints are case-scoped (`?case_id=`) like the rest of the app.
