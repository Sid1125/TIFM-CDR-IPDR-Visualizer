...
### i9 CDR-Analyzer Parity Pack (field-investigator features)
Closing the bread-and-butter gaps vs the commercial i9 CDR Analyzer, in 8 phases:
- **Excel / .xls(x) / .txt import:** uploads dispatch on filename extension via `_read_table()` (csv, txt delimiter-sniffed, xls via `xlrd`, xlsx via `openpyxl`) across CDR/IPDR/tower/dump/SDR importers and `/upload/preview`. (`api/upload.py`, `requirements.txt`)
- **Telecom reference lookups:** offline `/reference` over bundled datasets — comprehensive ISD codes (country, longest-prefix), India number-series→operator/circle and IMEI TAC→make/model (extensible seeds, degrade to Unknown). Client caches series/isd/tac via `/reference/meta`; `refOperator/refCircle/isIsdNum/refCountry/refImei` enrich the profile and reports. (`services/reference_service.py`, `api/reference.py`, `app/data/{isd_codes,mobile_series,imei_tac}.json`)
- **CDR analysis-report pack:** per-subject **Analysis Reports** tab — day first/last call, single-call days, weekday/weekend, longest calls, day/night summary (+dominant tower), ISD calls, other-state calls (series→circle), OFF/unused periods, IMEI summary (+make/model), SIM/IMSI summary, bank/OTP-sender SMS. Pure client-side over `allRows`; every table exports CSV/XLSX. (`renderAnalysisReports()`)
...
### Investigation Workspace — relationship labels & hypotheses
- **Description:** Two additions to the workspace layer (evidence pins, subject intel tags, annotations and the audit trail already existed). **Relationship labels** put a label on the link *between* two subjects ("brothers", "supplier→dealer", "same vehicle") — global by pair, stored normalised so it's order-independent, so a known relationship follows the two identifiers across every case. **Hypotheses** are a structured "theory of the case": a titled hypothesis with a body, a status (open/supported/refuted) and linked subjects, case-scoped. Both are managed from the Evidence tab and every mutation is written to the chain of custody.
- **Tech:** `models/relationship_label.py` + `models/hypothesis.py`; `api/workspace.py` (`GET/PUT /relationships`, `GET/POST/PUT/DELETE /hypotheses`); frontend `renderHypotheses()`/`renderRelationships()` panels in the Evidence tab. Tests: `test_workspace.py`.

### Server-Side Graph Layout (cached)
- **Description:** For large graphs the node positions are computed **on the server** so the browser just draws them instead of running its own force simulation. `compute_layout()` is a pure-Python grid-approximated Fruchterman-Reingold layout (no SciPy) with grid-bucketed repulsion (≈O(n) per iteration — ~3s for 1.5k nodes, ~10s for 3k); the result is cached in `analytics_cache` (cleared with the case on any data change), making it a one-time cost. `GET /graph/?layout=1` attaches server `x`/`y` to each node; the canvas renderer scales them into the viewport, pins them, and draws statically. Requested automatically for the canvas path (limit 0 or ≥700 nodes); small graphs keep the client D3 layout.
- **Tech:** `graph_service.py` (`compute_layout`, `cached_layout`, `build_graph_with_layout`), `api/graph.py` `layout` param, `_renderGraphCanvas()` server-position path. Tests: `test_graph_metrics.py` (`ServerLayoutTests`).

### Windows Installer / Packaging
- **Description:** A packaging path that turns ARGUS into a one-click **`ARGUS_Setup.exe`** (install → run → done) instead of clone → Python → Postgres → Redis. Bundles the FastAPI app + Python + the frontend via PyInstaller and runs **zero-config against SQLite** (fully self-contained / air-gapped); dropping a `.env` with `DATABASE_URL` opts into PostgreSQL, auto-detected by the capability layer. `main.py`'s `static_dir` is frozen-aware so the bundled UI is found.
- **Tech:** `packaging/launch.py` (frozen entry point), `packaging/argus.spec` (PyInstaller one-dir), `packaging/build.ps1`, `packaging/argus_installer.iss` (Inno Setup), `packaging/README.md`.

---

## 23. Entity Resolution & Intelligence Graph

ARGUS's analytics historically reasoned about *identifiers* — a phone number, an IMSI, an IMEI, an IP address — as unrelated strings. This layer resolves identifiers into **entities**: the probable person, device, or device-cluster behind them, so an investigator can ask "who is this?" instead of chasing four disconnected values by hand. Resolution is entirely deterministic (union-find over same-record co-occurrence) — **no LLM ever decides who is who**; AI may summarise the result elsewhere but never performs the linking.

### Entity Resolution Engine (identifiers → entities)
- **Description:** MSISDN / IMSI / IMEI identifiers seen together **on the same record** are bound via union-find (that row physically witnessed the SIM in that device answering that number); the binding is transitive on purpose — the same IMEI carrying a new IMSI is a SIM swap, the same IMSI moving to a new IMEI is a device change, and both stay **one entity**, surfaced as flags (`sim_swap`, `device_change`, `multiple_numbers`, `multi_case`, `device_reuse`) rather than hidden. **IP addresses are attributes, never merge keys** — carrier/CGNAT addresses are shared and reassigned, so merging on them would fuse strangers. **Counterpart (b-party) numbers are not entity members** — they become communication edges between entities (or to an outside/unresolved number). A **fan-out guard** stops a placeholder/shared identifier (a blank IMEI, a test SIM, a device-farm handset) from bridging hundreds of unrelated subscribers into one blob: the threshold is **learned per case** from the distribution of per-identifier fan-out using a Tukey far-outlier fence (Q3 + 3·IQR — a fixed percentile was rejected because on a small bimodal sample it just returns the outlier itself), clamped to a sane floor/ceiling, and surfaced in the API so it's never a black box. Every entity is classified — `individual` / `linked_identity` / `identity_cluster` (device farm, SIM-box, shared handset, organisation) / `identifier` — **never assumed to be one person**. Every binding link carries typed evidence (Number↔SIM / Number↔Device / SIM↔Device), a witnessing record count, a co-occurrence time window, the endpoints' fan-out, a HIGH/MEDIUM/LOW confidence tier, and a **deterministic plain-language explanation** ("This phone number and SIM appeared together in 9,003 telecom records between 01 Jun 2026 and 30 Jun 2026… a strong identity relationship") — fully court-explainable, no black-box merge.
- **Tech:** `services/entity_service.py` — `_UnionFind`, `_idents()`, `_adaptive_hub_fanout()` (Tukey IQR fence), `_link_confidence()`, `_entity_classification()`, `_explain_link()`, `build_entities()`, `resolve_entities()`. Entity id = stable SHA1 hash of the sorted member-identifier set, so it survives rebuilds unchanged as long as membership doesn't change. Tested in `tests/test_entities.py` (same-record binding, SIM-swap/device-change flagging, unrelated subjects stay separate, CGNAT never merges, counterpart-as-edge, adaptive threshold learns the case's own normal vs. catching a placeholder hub, deterministic explanation text).

### Persistent Entity Intelligence Graph (models, sync, decisions)
- **Description:** The in-memory resolution engine stays a pure function; a persistence layer materialises its output so the graph **survives restarts**, is **queryable at scale** (pagination, search, ego-graphs) without re-resolving on every request, and carries **investigator judgement forward forever**. A sync only re-resolves a scope (one case, or the whole database) when the underlying record count or decision count actually changed — cheap freshness check first. Investigator **merge decisions are durable**: reject a suggested/binding merge and that identifier pair is **never unioned again**, even across a forced rebuild; confirm one and it merges even through the fan-out guard. Every relationship persisted answers "why" — `CONTACTED` (call/SMS evidence count), `USES_SERVICE` (attributed app + confidence), `APPEARS_IN_CASE`, `CO_LOCATED` (shared towers — **never a merge signal**, purely circumstantial), `POSSIBLE_ASSOCIATION` (co-location **plus** direct communication — still not identity, just upgraded caution), and `SUGGESTED_MERGE` (a fan-out-guard-blocked pair whose endpoints landed in different entities, with a plain-language reason ARGUS withheld the automatic merge). Entity **confidence (0–100)** is deterministic: the weakest binding link defines the whole cluster's integrity (a chain is its weakest link), with a small volume bonus, a device-reuse penalty, and a bonus when a human has confirmed a binding.
- **Tech:** `models/entity.py` — `Entity` (uid/type/classification/confidence/review state + lossless JSON payload), `EntityIdentifier` (typed MSISDN/IMSI/IMEI/IP/APP/TOWER rows, indexed for search), `EntityRelationship`, `EntityMergeDecision` (durable verdicts keyed by a stable identifier pair-key), `EntitySyncState` (count-based freshness marker). `services/entity_graph_service.py` — `sync_entities()`, `entity_confidence()`, ego-graph builder, `search_entities()`, `entity_cross_case()`. Tested in `tests/test_entity_graph.py` (sync persists identifiers, same-IMEI links two numbers, SIM-swap flag persists, tower-only co-location never merges, co-location+contact upgrades to possible-association, rejected-merge stays rejected across a forced rebuild, confirmed-merge survives the fan-out guard, confidence formula bounds/weakest-link/device-reuse-penalty, review lifecycle + chain-of-custody).

### Investigation Ego-Graph & Review Workflow (Entities tab)
- **Description:** The Entities tab centres a radial **investigation graph** on one entity — phones, SIMs, devices, IP endpoints, attributed services, towers, cases, and related entities as typed, colour-coded nodes, with confidence-tinted edges. Hovering an edge shows its deterministic evidence prose; clicking a related-entity node pivots the view onto it. The graph is **depth-1 and node-capped server-side** (never the whole database in the browser) — capped groups show a "+N more" note, and each entity fetch is a fresh bounded call, so this scales to 1M+ record cases. A **review bar** lets an investigator move an entity through SYSTEM → CONFIRMED / FALSE MERGE (with un-review), a **suggested-merge queue** surfaces every fan-out-guard-blocked pairing with one-click Confirm/Reject, and each **binding-evidence row** carries a per-pair "split" button to reject just that one link without discarding the rest of the cluster. Every verdict is durable (feeds back into the next resolution) and chain-of-custody logged.
- **Tech:** `static/entities/entities.js` — `_egoSvg()`/`_wireEgo()` (radial layout, typed edge colours/dashing), `_reviewBar()`, `_suggestionQueue()`, `_review()`. Backend `api/entities.py` — `GET /entities/` (paginated/filtered), `GET /entities/search`, `GET /entities/{uid}`, `GET /entities/{uid}/graph`, `POST /entities/{uid}/review` (`confirm_merge`/`reject_merge`/`confirm`/`reject`/`unreview`/`note`).

### Entity-Level Cross-Case Intelligence
- **Description:** Upgrades cross-case linking from "this **identifier** exists in another case" to "this **entity** appears in another case" — so a suspect using a *different* phone number in each case but the *same handset* (IMEI) or *same SIM* (IMSI) still surfaces as **one** cross-case hit, with the per-case identifier breakdown as evidence and a one-line deterministic finding ("The same handset (IMEI …) appears in 2 cases — likely the same physical device/operator, even where the numbers differ"). Surfaced as a **"Recurring entities"** panel on the Cross-Case tab (alongside the existing per-identifier list), each card deep-linking into the Entities tab; a deep-link falls back to the all-cases resolution when the entity only exists at global scope. Case labels are resolved to their actual **names** everywhere (not raw numeric ids), and the underlying identifier-level matching was fixed to also recognise phone subjects from **IPDR-only** cases (it previously only scanned CDR rows, so a case with zero CDR records could never surface its own cross-case hits).
- **Tech:** `entity_cross_case()` + `_cross_case_finding()` + `_case_names()` in `entity_graph_service.py`; `GET /entities/cross-case`. Frontend `_renderXcEntities()` in `analytics/crosscase.js`. `cross_case_service.py` — `subject_cross_case`'s phone-subject check and `case_cross_case_overview`'s number-seeding now also query `IPDRRecord.msisdn`, not just CDR. Tested in `tests/test_entity_graph.py` (`CrossCaseEntityTests` — same-device-different-numbers match, single-case entities excluded).
