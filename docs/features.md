...
- **Description:** A **Dossier** button in the top bar generates a formal, government-style case report an investigating officer can **Print → Save as PDF** and attach to a charge sheet — fully offline (air-gapped; no PDF library bundled). The document is laid out as an official record: an **ARGUS all-seeing-eye emblem letterhead** with a configurable **agency / unit name + sub-line** (and optional agency logo), top and bottom **RESTRICTED — FOR OFFICIAL USE ONLY** classification banners, a titled cover with a metadata table (document reference, case, prepared-by + role, generated-at, evidence window, records/subjects/towers, classification) and a **Document Control** statement. Body sections: a **Contents** page, **1. Executive Summary** (key counts, persons of interest, meetings, impossible-travel flags, cross-case links, bookmarked-evidence count), **2. Case Narrative** (auto-reconstructed story for the principal subject), **3. Subjects of Interest**, **4. Communication Analysis**, **5. Cross-Case Links** (from `/cross-case/report`), **6. Bookmarked Evidence** (the evidence folder), **7. Analytical Charts** (live Chart.js canvases rasterized via `canvas.toDataURL()`), **8. Tower Index**, and an **Appendix A — Methodology & Limitations**. The print stylesheet hides all app chrome, sets A4 margins, repeats the classification + **page X of Y** in the page footer (CSS `@page` margin boxes), and avoids splitting tables/figures. Generating a dossier is **logged** (ExportLog + AuditLog) under the official reference id on the cover.
- **Tech:** Frontend `renderDossier()` (composes from `state`, `meetingTotals()`, `getInfReport()`, `getStoryXcase()`, `buildCaseEvents()`/`buildNarrative()`, `evLoad()`) + `fillDossierXcase()`; inline `ARGUS_EMBLEM` SVG; `getAgency()`/`setAgencyDetails()` (localStorage `argus_agency`, "Agency details" toolbar button); `#dossier` overlay + official `.dc-*`/`.dossier-*`/`.d-*` styles + `@media print` (`@page` footer counters) in `styles.css`; `POST /inference/dossier` for the official ref + chain-of-custody logging.
...
### Evidence Linking (bookmark → folder → dossier)
- **Description:** Any finding on the Story timeline — a hidden link, SIM swap, meeting, cross-case match, or AI insight — can be **pinned (☆)** into a per-case **Evidence Folder**. The folder is a slide-out panel (with a live count badge) listing each bookmarked finding with its type, detail, subject and timestamps, and a remove control; "Clear" empties it. The evidence folder is **automatically included as a section in the court-ready dossier**, so the analyst curates the key findings once and they flow straight into the report.
- **Tech:** Frontend `evLoad()`/`evSave()`/`pinEvidence()`/`unpinEvidence()` (per-case `localStorage` key `argus_evidence_{caseId}`, deduped by a stable event signature), `renderEvidence()`, pin toggles on every timeline event; surfaced in the dossier's **Bookmarked Evidence** section. `.evidence-*` styles.
