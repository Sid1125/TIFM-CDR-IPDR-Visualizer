...
- **Description:** Suspects reoffend, but every other view in ARGUS is single-case scoped, so a subject's history in *other* cases was invisible. Cross-case linking detects when a subject in the current case (or the handset/SIM behind it, or its IP) also appears in **previously loaded cases**, so the LEA sees prior history the moment a case is opened.
  ...
  - **Three surfaces:** (1) a **dashboard "Cross-case hits" panel** listing this case's subjects that recur elsewhere, sorted by other-case count, each opening the subject profile; (2) a **profile-modal panel** ("Also seen in N other cases") with one clickable chip per case showing match type, confidence, record count and date span — clicking jumps to that case and reopens the subject there; (3) a dedicated **Cross-Case tab** (see below) with the full dossier.
- **Cross-Case Tab (full dossier):** A dedicated tab giving the complete cross-reference of the current case against **all** other loaded cases: a headline summary (recurring subjects, linked cases, high/low-confidence counts, strongest-link-per-subject mix, total subject↔case link pairs), a **per-linked-case rollup** (each other case with its shared subjects, match-type badges, confidence, role and record counts), and a **per-recurring-subject** expandable list (every case the subject appears in, with the exact matched identifiers, role, record count and date span). Every subject opens its profile; every case chip jumps to that case (optionally landing on the subject). Match types are colour-badged (number / IMEI / IMSI / IP). Refresh button recomputes.
- **Tech:** Server-side `backend/app/services/cross_case_service.py` (`subject_cross_case`, `case_cross_case_overview`, `case_cross_case_report`) — set-based, index-backed, chunked `IN()` lookups over the per-column identity indexes (`msisdn`/`imei`/`imsi`/`a_party_number`/`source_ip`), no full-case load. Endpoints `GET /cross-case/subject`, `GET /cross-case/overview`, `GET /cross-case/report`. Frontend `renderCrossCaseHits()`, `fillProfileCrossCase()`, `renderCrossCaseTab()`/`renderCrossCaseReport()`, `openInCase()`; `.case-link` / `.xcase-*` / `.xc-*` styles reuse the `twr()` chip pattern. Tested in `backend/tests/test_cross_case.py` (number + SIM-swap-by-IMEI links, isolated-subject negative, IP low-confidence, CDR/IPDR separation, overview counts, report aggregation/rollups).
